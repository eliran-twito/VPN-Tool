...
class DebugSSH(object):
    ...
    def general_debug(self):
        ...
        try:
            ...
            ssh_client.exec_command('cat /opt/CPsuite-R81.20/fw1/log/fwk.elg /opt/CPsuite-R81.20/fw1/log/fwk.elg.0 /opt/CPsuite-R81.20/fw1/log/fwk.elg.1 | grep -i "fail" > /var/log/VPN_Tool/DB/fwk_failures_DB.txt; sort -k2M /var/log/VPN_Tool/DB/fwk_failures_DB.txt -o /var/log/VPN_Tool/DB/fwk_failures_DB.txt')
            ssh_client.exec_command('cat /opt/CPsuite-R81.20/fw1/log/fwk.elg /opt/CPsuite-R81.20/fw1/log/fwk.elg.0 /opt/CPsuite-R81.20/fw1/log/fwk.elg.1 | grep -i "error" > /var/log/VPN_Tool/DB/fwk_error_DB.txt; sort -k2M /var/log/VPN_Tool/DB/fwk_error_DB.txt -o /var/log/VPN_Tool/DB/fwk_error_DB.txt')
            ...
            ssh_client.exec_command(
                ...
                'epoch_time=$('
                'date -d \"$timestamp\" \"+%s\"); if [[ \"$epoch_time\" -ge \"$start_time\" && \"$epoch_time\" -le '
                '\"$end_time\" ]]; then echo \"$line\"; fi; done < \"/var/log/VPN_Tool/DB/fwk_failures_DB.txt\"> '
                '/var/log/VPN_Tool/fwk_failures.txt')
            ssh_client.exec_command(
                'start_time=$(date -d \"'+ self.F_date +''+self.F_time+'\" \"+%s\"); end_time=$(date -d \"'+ self.T_date +''+self.T_time+'\" \"+%s\");while '
                'read -r line; do timestamp=$(echo \"$line\" | awk \'{print $1\" \"$2\" \"$3\" \"$4}\');'
                'epoch_time=$('
                'date -d \"$timestamp\" \"+%s\"); if [[ \"$epoch_time\" -ge \"$start_time\" && \"$epoch_time\" -le '
                '\"$end_time\" ]]; then echo \"$line\"; fi; done < \"/var/log/VPN_Tool/DB/fwk_error_DB.txt\"> '
                '/var/log/VPN_Tool/fwk_error.txt')
